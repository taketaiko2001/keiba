...
- **代替データ源`tools/yahoo_keiba_odds.py`を追加(2026-09-14・要ライブ検証)**: 運用者の指示で無料の代替オッズ提供元を調査し、Yahoo!スポーツ競馬(`sports.yahoo.co.jp/keiba/race/odds/tfw/<yahoo_race_id>`)がログイン不要の一般公開ページとして単勝・複勝オッズを静的HTMLに直接埋め込んでいることを発見した(netkeibaのようなJS/Ajax後付けではない)。`yahoo_race_id`はnetkeiba形式race_id(12桁)の先頭2桁("20")を除いた10桁。この専用ツールを追加し、確定済み(レース終了後)のレースで正しい単勝・複勝オッズが取得できることを確認した。**ただし発売中(発走前)のライブオッズでも同様に取得できるかは2026-09-14時点で未検証**(検証を試みた時点でJRAは非開催日だった)。次回のJRA開催日(2026-09-19・20予定)の巡回サイクルで、発売中のレースに対して`tools/yahoo_keiba_odds.py`を実行し実オッズが返るかを最優先で確認し、記録.mdに結果を明記すること。**このツールを使うには運用環境(keiba-env2)のNetwork accessの許可リストに`sports.yahoo.co.jp`を追加する必要があり、2026-09-14時点では運用者にまだ依頼中(未許可)。許可されるまでは接続エラーになるため、CLAUDE.mdのデータ取得優先順位2番として位置付けつつも、実際に使えるかは許可状況次第。** もし発売中も正しく取得できれば、2026-09-01のJRA限定運用開始以降続いていた新規購入0件の状態から復帰できる見込み。
